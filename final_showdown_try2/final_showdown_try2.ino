...
void RFIDProcessing(){
  ...
  //verifying and pushing to firebase
...
void MQ2Processing(){
  ...
  if (analogSensor > sensorThres){
    ...
    Serial.println("ppm");
    String name = Firebase.pushFloat("MQ2SensorValue", analogSensor);
  }
  // handle error
  if (Firebase.failed()) {
    Serial.println(Firebase.error());
...
void LoadCellProcessing(){
  ...
  String name = Firebase.setFloat("LoadCellValue", noOfUnits);
  // handle error
  if (Firebase.failed()) {
    Serial.println(Firebase.error());
    Serial.print("setting /number failed:");
  ...
  Serial.print("pushed: /LoadCellValue/");
  Serial.println(noOfUnits);
  delay(500);
}
void loop() {
  ...
  if (!mfrc522.PICC_IsNewCardPresent())
    return;
  // Select one of the cards Reset the loop if no new card present on the sensor/reader. This saves the entire process when idle.
